...
**由于 AOSP 镜像造成CPU/内存负载过重，我们限制了并发数量**。
sync的时候并发数不宜太高，否则会出现 503 错误，即`-j`后面的数字不能太大，建议选择4。
<!--请尽量选择流量较小时错峰同步。-->
...
你希望以后通过 NJU 同步 AOSP 部分的代码，只需要修改 `.repo/manifests.git/config`，将
